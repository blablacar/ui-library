...
const Message = ({
  active, author, date, isoDate, secondaryLink, secondaryText, children, className,
}: MessageProps) => (
  <blockquote className={cc(['kirk-message', prefix({ active }), className])}>
    { author && <cite>{author}</cite> }
    <div className="kirk-label">
      <p>{children}</p>
      <Caption href={secondaryLink} secondaryText={secondaryText} isoDate={isoDate}>{date}</Caption>
    </div>
