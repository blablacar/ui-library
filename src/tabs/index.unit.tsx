...
describe('Rendering testing', () => {
    it('should render properly', () => {
        const tabsAsJson = renderer
            .create(
                <Tabs tabs={defaultTabsConfig.tabs}
                      activeTabId={defaultTabsConfig.activeTabId}></Tabs>,
            )
            .toJSON()
...
describe('ARIA roles', () => {
    it('Should generate proper ARIA roles', () => {
        ...
        const activeTab = tablist.find('[role="tab"].kirk-tab-selected')
        expect(activeTab.exists()).toBe(true)
        expect(activeTab.length).toBe(1)
        expect(activeTab.get(0).props['aria-selected']).toBe('true')
    })
    ...
    it('Should update ARIA attributes after switching tab', () => {
        ...
        // Verify that the current tabpanel id is correct.
        tabpanel = wrapper.find('[role="tabpanel"]')
        expect(tabpanel.get(0).props.id).toBe('tab2_panel')
    })
})
...
describe('Keyboard navigation', () => {
    ...
    it('goes to first tab when going right after the last tab', () => {
        ...
        const wrapper = mount(
            <Tabs tabs={defaultTabsConfig.tabs} activeTabId='tab3'></Tabs>,
        )
